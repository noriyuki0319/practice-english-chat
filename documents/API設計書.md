...
| `POST`        | `/api/chat-groups`         | チャットグループ作成               | ユーザーメッセージ送信時に自動作成     |
| `POST`        | `/api/chat-messages/user`  | ユーザーメッセージ保存             | チャットグループIDと共に保存           |
| `POST`        | `/api/chat-messages/ai`    | AIメッセージ保存                   | ストリーミング完了後に保存             |
| `GET`         | `/api/bookmarks`           | ブックマーク一覧取得               |                                        |
| `POST`        | `/api/bookmarks`           | ブックマーク作成                   | チャットメッセージIDを指定             |
...
### 4.2. チャットグループ作成とユーザーメッセージ保存

ユーザーがチャットメッセージを送信する際、新しいチャットグループを作成し、そのグループにユーザーメッセージを保存します。

- **メソッド**: `POST`
- **パス**: `/api/chat-groups`
- **説明**: ユーザーの最初のメッセージ送信時に、新しいチャットグループを作成し、ユーザーメッセージを`chat_messages`テーブルに保存します。グループのタイトルはユーザーメッセージの最初の30文字から自動生成されます。
...
### 4.3. ユーザーメッセージ保存（既存グループ）

既存のチャットグループにユーザーメッセージを追加します。

- **メソッド**: `POST`
- **パス**: `/api/chat-messages/user`
- **説明**: 既存のチャットグループに対して、ユーザーの新しいメッセージを保存します。
...
| `400 Bad Request`           | リクエストボディが不正               |
| `401 Unauthorized`          | 認証エラー                           |
| `403 Forbidden`             | 他のユーザーのグループへのアクセス   |
| `404 Not Found`             | 指定されたチャットグループが存在しない |
| `500 Internal Server Error` | サーバー内部エラー                   |
...
AIからのレスポンスを`chat_messages`テーブルに保存します。

- **メソッド**: `POST`
- **パス**: `/api/chat-messages/ai`
- **説明**: ストリーミングが完了した後、AIの各提案メッセージを`chat_messages`テーブルに保存します。`user_id`は`NULL`、`role`は`'ai'`として保存されます。
...
AIが生成したメッセージ（`chat_messages`）をブックマークとして保存します。

- **メソッド**: `POST`
- **パス**: `/api/bookmarks`
- **説明**: ユーザーがブックマークアイコンをクリックした際、該当するAIメッセージを`bookmarks`テーブルに保存します。
...
| `400 Bad Request`           | リクエストボディが不正                   |
| `401 Unauthorized`          | 認証エラー                               |
| `404 Not Found`             | 指定されたチャットメッセージが存在しない |
| `409 Conflict`              | 既にブックマーク済み                     |
| `500 Internal Server Error` | サーバー内部エラー                       |
